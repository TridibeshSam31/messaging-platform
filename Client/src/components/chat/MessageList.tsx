...
export function MessageList({ conversationId }: Props) {
  const { messages, cursors } = useChatStore()
  ...
  const prevFirstMsgId = useRef<string | undefined>(convMessages[0]?.id)
  ...
  useLayoutEffect(() => {
    const container = scrollContainerRef.current
    if (container && scrollHeightRef.current && !atBottom && convMessages.length > prevMessagesLength.current) {
      const firstMsgIndex = convMessages.findIndex(m => m.id === prevFirstMsgId.current)
      if (firstMsgIndex > 0) {
  ...
  useEffect(() => {
    ...
    if (!el || !hasMore || loadingOld) return
